...
if (isset($_POST) && $_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $vpb_with_of_first_image_file = 200;      // You may adjust the width here as you wish
    $vpb_with_of_second_image_file = 200;     // You may adjust the height here as you wish

    /* Variables Declaration and Assignments */
    $vpb_image_filename = $_FILES["browsed_file_banner2"]["name"];
    $vpb_image_tmp_name = $_FILES['browsed_file_banner2']['tmp_name'];
    $vpb_file_size = filesize($_FILES['browsed_file_banner2']['tmp_name']);
    ...
    if ($vpb_image_filename == "") {
        ...
        if ($vpb_file_extensions != "gif" && $vpb_file_extensions != "jpg" && $vpb_file_extensions != "jpeg" && $vpb_file_extensions != "png" && $vpb_file_extensions != "GIF" && $vpb_file_extensions != "JPG" && $vpb_file_extensions != "JPEG" && $vpb_file_extensions != "PNG") {
            ...
            if ($vpb_image_src == "invalid_file_type_realized") {
                ...
                echo '<div class="sliki"><div class="image-cropper  img-polaroid"><input type="hidden" name="slika_baner2" value="' . $vpb_uploaded_file_movement_one . '"><input type="hidden" name="slikathumb" value="' . $vpb_uploaded_file_movement_two . '"><img class="centered" src="' . $vpb_uploaded_file_movement_two . '"></div></div>';
            }
        }
    }
}
